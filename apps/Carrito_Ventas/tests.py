...
class FacturaTestCase(TestCase):
    ...
    def Test_listar_factura(self):
        ...
        u1 = Usuario.objects.get(password="1234",is_superuser=1,username="samuel",first_name="samuel",last_name="rosales",email="samuel@mail.com",is_staff=1,is_active=1,date_joined=datetime.now(),tipo=1)
        self.user = u1
        ca = Carrito.objects.create(usuario_fk=u1,monto_a_pagar=500)
